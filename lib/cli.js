...
const u4pm_jaydata_dynamic_metadata_1 = require("u4pm-jaydata-dynamic-metadata");
const fs = require("fs");
...
if (argv.help)
    ...
    if (argv.metadataUri.indexOf('http:') == 0 || argv.metadataUri.indexOf('https:') == 0) {
        var dynamicMetadata = new u4pm_jaydata_dynamic_metadata_1.DynamicMetadata({});
        ...
        fs.readFile(argv.metadataUri, 'utf8', function (err, text) {
            if (err) {
                ...
                var $metadata = text;
                process(new u4pm_jaydata_dynamic_metadata_1.MetadataHandler({}, {
                    debug: true,
                    autoCreateContext: !argv.autoCreateContext ? undefined : argv.contextInstanceName || 'context',
                    namespace: argv.namespace,
                    contextName: (argv.dts || 'JayDataContext.d.ts').split('.')[0],
